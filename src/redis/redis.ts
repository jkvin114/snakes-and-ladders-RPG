...
export namespace RedisClient{
    ...
    export async function removeFromSet(key:string,...val:string[]) {
    ...
    export async function countSet(key:string) {
    ...
    export async function isInSet(key:string,val:string) {
        if(!redisClient) return false
        try{
           return await redisClient.sismember(key,val)
        }
        catch(e){
            Logger.error('Redis Client Error', e)
            return false
