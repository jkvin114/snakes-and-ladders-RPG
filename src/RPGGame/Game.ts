...
import { GameRecord } from "./Simulation/data/GameRecord"
...
class Game {
	...
	constructor(mapid: number, rname: string, setting: GameSetting) {
		...
		this.entityMediator.registerGameEventObserver(this.eventEmitter)
		this.trainLabels=[]
	...
	registerGameEventObserver(ci:GameEventObserver){
		...
		this.entityMediator.registerGameEventObserver(ci)
	}
	onCreate(){
	...
	goNextTurn():ServerGameEventFormat.TurnStart|null {
		...
		if (p.effects.has(EFFECT.DOUBLEDICE)) {
			effects.push("doubledice")
		}
		if (p.effects.has(EFFECT.BACKDICE)) {
			effects.push("backdice")
		}
		if (p.effects.has(EFFECT.CURSE)) {
			effects.push("badluck")
		}
		let noDice=p.effects.has(EFFECT.ROOT) || p.effects.has(EFFECT.GROUNGING)
		...
		if (p.effects.has(EFFECT.SLOW)) {
			additional_dice -= 2
		}
		if (p.effects.has(EFFECT.SPEED)) {
			additional_dice += 2
		}
	...
	//	p.onBeforeObs()
		...
		// //	p.effects.apply(EFFECT.SILENT, 1, EFFECT_TIMING.BEFORE_SKILL)
		// 	if (p.AI) {
		// 		p.AiAgent.store()
