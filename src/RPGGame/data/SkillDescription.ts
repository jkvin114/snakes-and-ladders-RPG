...
export class SkillInfoFactory {
	...
	private getQ() {
		...
		switch (this.char) {
			...
			case 2:
				data=[
					this.mDmg(this.baseDmg(s), hotkey),this.effect(EFFECT.BLIND, 1)
				]
				break
			case 3:
				data=[
			...
			case 4:
				data=[
					this.pDmg(this.baseDmg(s), hotkey),this.effectNoDur(EFFECT.ROOT),
					this.effectNoDur(EFFECT.GROUNGING),
		...
		switch (this.char) {
			...
			case 2:
				str =
					this.nameTitle(s) +
					this.target +
					`에게 ${this.mDmg(this.baseDmg(s), hotkey)}를 입히고 ${this.effect(EFFECT.BLIND, 1)} 부여`
				break
			case 3:
				str =
					this.nameTitle(s) +
			...
			case 4:
				str =
					...
					this.target +
					`에게 총을 발사해 
				${this.pDmg(this.baseDmg(s), hotkey)}를 입힘, ${this.effectNoDur(EFFECT.ROOT)} 혹은 ${this.effectNoDur(EFFECT.GROUNGING)}
	...
	private getW() {
		...
		switch (this.char) {
			...
			case 1:
				str =
					this.nameTitle(s) +
					`Leaves a ${this.emp("mark")} to a ${this.target} and applies ${this.effect(EFFECT.CURSE, 1)}`
				break
			case 2:
				str =
					this.nameTitle(s) +
					this.passive() +`If you have`+
					this.effectNoDur(EFFECT.INVISIBILITY) +
			...
			case 3:
				...
				${this.active()}${this.heal(this.skillAmt("wheal")*3, "wheal")} total for ${this.duration(3)} ,
				 receives ${this.effectNoDur(EFFECT.SLOW)} effect while healing.`
				break
			case 4:
				str =
					this.nameTitle(s) +
					` ${this.proj("trap")} of ${this.projsize(3) } that applies ${this.effect(EFFECT.GROUNGING, 1)}
					 to the enemy who steps on it`
				break
			case 5:
				str =
					this.nameTitle(s) +
					`Gain ${this.effect(EFFECT.ROOT, 1)} and ${this.shield(50)}. ${this.rangeStr()} of ${
			...
			case 6:
				str =
					this.nameTitle(s) +
					`Deals ${this.mDmg(this.baseDmg(s), hotkey)} and applies ${this.effect(EFFECT.SLOW,1)} to enemies ${this.radius(3)}.
			...
			case 7:
				str =
				 ...
				 and applies ${this.effect(EFFECT.ROOT, 1)} `
				break
			case 8:
				str =
					this.nameTitle(s) +
					`${this.proj("vine")} of ${this.projsize(1)} that ${this.emp("stops")} player who passes it,
				  allies will receive ${this.effect(EFFECT.SPEED, 1)}`
				break
				case 9:
					str =
					this.nameTitle(s) +this.passive()+`Deals additional  ${this.mDmg(this.baseDmg(s),hotkey)} on ${this.basicattack()}. 
					`+this.active()+
					`Select a ${this.target } and forcibly moves ${this.emp("2+(1 for 3 vulnerability stacks)squares")} backwards and applies ${this.effect(EFFECT.CURSE, 1)}.
					`
					break
	...
	private getWKor() {
		...
		switch (this.char) {
			...
			case 1:
				str =
					this.nameTitle(s) +
					`사용시 ${this.target}에게 ${this.emp("표식")}을 남기고 ${this.effect(EFFECT.CURSE, 1)} 부여`
				break
			case 2:
				str =
					this.nameTitle(s) +
			...
			case 3:
				...
				${this.active()}${this.duration(3)}에 걸쳐 ${this.heal(this.skillAmt("wheal")*3, "wheal")},
				 회복 중엔 ${this.effectNoDur(EFFECT.SLOW)}에 걸림.`
				break
			case 4:
				str =
					this.nameTitle(s) +
					this.projsize(3) +
					`의 ${this.proj("덫")}해
				 밟은 적에게 ${this.effect(EFFECT.GROUNGING, 1)} 부여`
				break
			case 5:
				str =
					this.nameTitle(s) +
					`사용시 ${this.effect(EFFECT.ROOT, 1)}, ${this.shield(50)} 후 ${
			...
			case 6:
				str =
				 ...
				 입히고 ${this.effect(EFFECT.SLOW,1)} 부여. 자신은 ${this.shield(this.skillAmt("wshield"), "wshield")}`
				break
			case 7:
				str =
					this.nameTitle(s) +
				...
				${this.basicattack()}시 ${this.mDmg(this.skillAmt("w_aa_adamage"), "w_aa_adamage")},
				 ...
				 를 추가로 입히고 ${this.effect(EFFECT.ROOT, 1)} `
				break
			case 8:
				str =
					this.nameTitle(s) +
					`지나가는 플레이어를 ${this.emp("멈추는")} ${this.projsize(1)}의 ${this.proj("덩굴")},
				 덩굴에 걸린 플레이어는 해당 칸의 효과를 받음, 아군은 ${this.effect(EFFECT.SPEED, 1)} 부여`
				break
			case 9:
				str =
					...
					`+this.active()+
					this.target +
					`을 선택해 뒤로 ${this.emp("2+(취약점 중첩 3당 1)칸")} 만큼 ${this.emp("강제이동")} 시키고 ${this.effect(EFFECT.CURSE, 1)} 부여.
					`
				break
			default:
				return ""
		...
		switch (this.char) {
			...
			case 1:
				...
				${this.active()} For ${this.duration(4)},  ${this.stat("Attack and magic resistance")} ${this.up(
					...
					)}, ${this.shield(this.skillAmt("rshield"),"rshield")},and gains ${this.effect(EFFECT.SPEED,1)
			...
			case 2:
				...
				${this.effectNoDur(EFFECT.SLOW)} and receives ${this.mDmg(this.baseDmg(s) * 3,hotkey)} for ${this.duration(3)}. `
				break
			case 3:
				str =
					this.nameTitle(s) +
			...
			case 4:
				...
				After use, you gains ${this.effect(EFFECT.DOUBLEDICE, 1)}.`
				break
			case 5:
				str =
					this.nameTitle(s) +
					`${this.proj("Magic circle")} of ${this.projsize(3)} that deals ${this.mDmg(this.baseDmg(s), hotkey)} and applies 
				${this.effect(EFFECT.SILENT, 1)} to enemy who step on it. Can use ${this.emp("3 times")}.`
				break
			case 6:
				str =
					this.nameTitle(s) +
			...
			case 7:
				...
				${this.basicattack()} deals additional ${this.pDmg(this.skillAmt("r_aa_adamage"), "r_aa_adamage")}.
				 ...
				 ${this.nameDesc(0)} creates an area of ${this.projsize(3)}. If enemy step on it,
				  they receive ${this.effect(EFFECT.IGNITE, 2)}`
				break
			case 8:
				str =
					this.nameTitle(s) +
				...
				${this.mDmg(this.skillAmt("plantdamage"), "plantdamage")} to enemies ${this.radius(
				 ...
				 ${this.effect(EFFECT.ROOT, 1)}.(2 turns if you are ${this.emp("Withered Tree")} state)
	...
	private getUltKor() {
		...
		switch (this.char) {
			...
			case 2:
				...
				${this.effectNoDur(EFFECT.SLOW)}에 걸리고 ${this.mDmg(this.baseDmg(s) * 3, hotkey)}를 받음`
				break
			case 3:
				str =
					this.nameTitle(s) +
			...
			case 4:
				...
				발사 후에는 ${this.effect(EFFECT.DOUBLEDICE, 1)}을 받음`
				break
			case 5:
				str =
					this.nameTitle(s) +
				...
				${this.effect(EFFECT.SILENT, 1)} 부여. ${this.emp("총 3번")} 시전할 수 있음`
				break
			case 6:
				str =
					this.nameTitle(s) +
			...
			case 7:
				...
				${this.pDmg(this.skillAmt("r_aa_adamage"), "r_aa_adamage")}를 추가로 입힘.
				 ...
				 ${this.nameDesc(0)} 적중 시 밟은 적에게 ${this.effect(EFFECT.IGNITE, 2)}
			...
			case 8:
				...
				${this.emp("식충식물")}은 ${this.duration(this.skillAmt("plant_lifespan"))}간 유지되며 매 턴마다 ${this.radius(
				 ...
				 ${this.effect(EFFECT.ROOT, 1)}.(${this.emp("시든 나무")} 상태이면 2턴)
