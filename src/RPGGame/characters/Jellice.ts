...
import { TickDamageEffect, TickEffect,ShieldEffect, NormalEffect } from "../StatusEffect"
...
class Jellice extends Player {
	...
	private getWShield() {
		return new ShieldEffect(EFFECT.MAGICIAN_W_SHIELD, 2, 50)
	}
	private getWEffect(){
		return new NormalEffect(EFFECT.MAGICIAN_W,2,EFFECT_TIMING.TURN_START)
	}
	private getWBurnEffect() {
		return new TickDamageEffect(
			EFFECT.MAGICIAN_W_BURN,
			2, //2
