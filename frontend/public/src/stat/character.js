...
async function showCharacterPage(version, map, gametype, charId, isModal, isback) {
	...
	try {
		data = await fetch(
			SERVER_URL + `/api/stat/eval/character/${charId}?version=${version}&map=${map}&gametype=${gametype}`
		)
		if (data.status === 400) {
			alert("invalid query")
		}
		...
		trend = await fetch(SERVER_URL + `/api/stat/eval/character/${charId}/trend?map=${map}&gametype=${gametype}`)
		if (trend.status === 400) {
			alert("invalid query")
		}
