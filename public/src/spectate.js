...
async function refreshGame(rname) {
	let game = await (await fetch("/room/rpg_game/" + rname)).json()
	if (!game) {
...
async function getlist() {
	let games = await (await fetch("/room/all_rpg_games")).json()
	let str = ""
...
$(document).ready(async function () {
	setting = await (await fetch("/resource/globalsetting")).json()
