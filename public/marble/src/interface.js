...
export class GameInterface {
	...
	init(setting, myturn) {
		...
		for (let i = 0; i < setting.players.length; ++i) {
			...
			$(this.doms.playerChar[ui_index]).attr(
				"src",
				"./../res/img/character/" +
					["reaper", "elephant", "ghost", "dinosaur", "sniper", "magician", "kraken", "bird", "tree"][
						p.char
					] +
					".png"
			)
	...
	setSavedCard(turn, name, level) {
        ...
        if(name!=="none" && name!=="" ){
            ...
            if(name==="angel_card" || name==="angel") $(this.doms.playerCard[ui_index]).find("img").attr("src","res/angel.png")
            if(name==="discount_card" || name==="discount") $(this.doms.playerCard[ui_index]).find("img").attr("src","res/coupon.png")
            if(name==="shield_card" || name==="shield") $(this.doms.playerCard[ui_index]).find("img").attr("src","res/shield.png")
