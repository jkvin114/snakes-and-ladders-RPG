...
const DEFAULT_AGENT=AgentType.SMART_CUSTOM_1
		...
		for (let i = 0; i < playerlist.length; ++i) {
			...
			if (this.game.gametype !== GameType.SOLOPLAY && 
				...
				if (agenttype === AgentType.RANDOM) {
					agent = new RandomAgent(playerstate, this.game, agenttype)
				} else if (agenttype === AgentType.SMART_CUSTOM_1) {
					agent = new CustomAgent1(playerstate, this.game, agenttype)
				} else {
					agent = new RationalRandomAgent(playerstate, this.game, AgentType.RATIONAL_RANDOM)
				}
				...
				this.aiCount += 1
				//set all players to playable in soloplay
			} else if (p.type === PlayerType.PLAYER_CONNECED || this.game.gametype === GameType.SOLOPLAY) {
				let agent = new RationalRandomAgent(
					game.state.CreatePlayer(),
					this.game,
