...
class MarbleGameMap {
	...
	setMap(map: any) {
		...
		for (const land of map.lands) {
			let tile = new LandTile(
				land.pos,
				TILE_TYPE.LAND,
				land.name,
				land.color,
				land.toll.map((t: number) => 10000 * t),
				land.price.map((t: number) => 10000 * t)
			)
		...
		this.corners
			.add(this.tiles[map.start])
			.add(this.tiles[map.island])
			.add(this.tiles[map.olympic])
			.add(this.tiles[map.travel])
	}
	onTurnStart(turn: number) {
		for (const tile of this.buildableTiles.values()) {
			if (tile.owner !== turn) continue
			if (tile.cooldownStatusEffect()) this.eventEmitter.setStatusEffect(tile.position, "", 0)
		}
	}
    cooldownPaint(turn: number) {
        let paintChanges:{owner:number,pos:number}[] = []
		for (const tile of this.buildableTiles.values()) {
			if (tile.owner !== turn) continue
            let paintOriginalOwner = tile.cooldownPaint()
			if (paintOriginalOwner!==-1) {
                paintChanges.push({owner:paintOriginalOwner,pos:tile.position})
            }
		}
        return paintChanges
	}
	...
	getTilesFromAnchor(source: MarblePlayer,anchorPos:number, ...filters: TileFilter[]): number[] {
		let result = new Set<number>()
		for (const f of filters) {
			let tiles: Set<number>
			if (f.buildableOnly) {
				tiles = this.filterFromBuildable(f, source,anchorPos)
			} else {
				tiles = this.filterFromAll(f, source,anchorPos)
			}
	...
	getTotalBuildPrice(lands: Iterable<number>) {
		...
		for (const pos of lands) {
			...
			if (tile) total += tile.getBuildPrice()
		}
		return total
	}
	...
	private filterFromBuildable(filter: TileFilter, source: MarblePlayer,anchorPos:number|null = null): Set<number> {
		let tiles = new Set<number>()

		if(anchorPos===null) anchorPos = source.pos

		for (const t of this.buildableTiles.values()) {
			if (filter.excludeMyPos && t.position === anchorPos) continue
			...
			if (distance(t.position, anchorPos) > filter.radius) continue
			if (filter.sameLine && !this.isSameLine(t.position, anchorPos)) continue
	...
	private filterFromAll(filter: TileFilter, source: MarblePlayer,anchorPos:number|null = null): Set<number> {
		let tiles = new Set<number>()
		if(anchorPos===null) anchorPos = source.pos
	...
	setLandOwner(tile: BuildableTile, owner: number) {
		...
		if (tile instanceof LandTile) this.updateColorMonopoly(tile, owner, prevOwner)
	...
	buildAt(tile: BuildableTile, builds: BUILDING[], player: number) {
		...
		if(builds.includes(BUILDING.LANDMARK)){
			this.eventEmitter.setTileState({pos:tile.position,state:"clear_paint",duration:0})
		}
		return price
	}
	updateColorMonopoly(tile: LandTile, newOwner: number, prevOwner: number) {
		...
		if (colors !== undefined && newOwner !== -1 && colors.every((tile) => tile.owner === newOwner)) {
			...
			if (tiles) tiles.forEach((land) => land.setColorMonopoly())
			// change[0]=color
		}
	}
	setMultiplierLock(pos: number) {
		if (this.lockedTile > -1) this.buildableTileAt(this.lockedTile)?.setMultiplierLock(false)
		this.lockedTile = pos

		if (pos !== -1) this.buildableTileAt(pos)?.setMultiplierLock(true)
	}
	...
	addSingleTileMultiplier(pos: number, count: number) {
		...
		this.eventEmitter.updateMultipliers([
			{
				pos: pos,
				mul: tile.getMultiplier(),
				toll: tile.getDisplayedToll(),
			},
		])
	}
	getLandToMoveMultiplier(invoker: MarblePlayer) {
		return this.getMostExpensiveIn(
			invoker,
			TileFilter.MY_LAND().setCondition((tile: Tile) => {
	...
	stealMultiplier(invoker: MarblePlayer, pos: number, dest: number) {
		...
		if (this.olympicPos === pos) this.setOlympic(dest)
		if (this.festival.has(pos)) this.setFestival(dest, pos)
	...
	removeStatusEffect(pos: number) {
		...
		if (!tile) return
		if (tile.removeStatusEffect()) this.eventEmitter.setTileState({ state: "remove_effect", pos: pos })
	}
	...
	checkMonopoly(changedTile: BuildableTile, invoker: number): MONOPOLY {
		...
		if (changedTile.type === TILE_TYPE.LAND) {
			if (countFrom(this.colorMonopolys.values(), (m: number) => m === invoker) >= 3) return MONOPOLY.TRIPLE
		}

		return MONOPOLY.NONE
	}
	checkAllMonopolyAlerts(invoker: number) {
		let sightcount = countFrom<number>(this.sights, (s) => this.tileOwners[s] === invoker)
		let alerts: MonopolyAlert[] = []
		if (sightcount === this.sights.length - 1) {
			alerts.push({ type: MONOPOLY.SIGHT, pos: this.sights.filter((s) => this.tileOwners[s] !== invoker) })
		}
        let linepos = []
		for (const line of SAME_LINE_TILES) {
			let linemissing = [...line].filter(
				(p) => this.tileAt(p) instanceof BuildableTile && this.tileOwners[p] !== invoker
			)
			if (linemissing.length === 1) {
                linepos.push(linemissing[0])
			}
		}
        if(linepos.length>0)
			alerts.push({ type: MONOPOLY.LINE, pos: linepos })
		...
		if (colorcount === 2) {
			for (const [color, lands] of this.sameColors.entries()) {
				...
				if (this.colorMonopolys.get(color) !== invoker) {
					...
					if (landsLeft.length === 1) {
                        triplepos.push(landsLeft[0].position)
                        
                    }
				}
			}
            if(triplepos.length>0){
                alerts.push({pos:triplepos,type:MONOPOLY.TRIPLE})
            }
		}

        //좋은 독점 먼저 오도록 정렬
        alerts.sort((a,b)=>b.type - a.type)

		return alerts
	}

	checkMonopolyAlert(changedTile: BuildableTile, invoker: number): MonopolyAlert {
		let pos: number[] = []
		//관독
		if (changedTile.type === TILE_TYPE.SIGHT) {
			let count = 0
	...
	updateTileState(state: GameState) {
		if (state.tiles.length === 0) {
			for (let i = 0; i < this.buildableTiles.size; ++i) state.CreateTile()
		}
		let i = 0
