...
def stable_trend_partition(offset,points,val1,val2,variance):
    ...
    # point_mean_values = get_partition_points(val2-val1,points.shape[0]+1,offset_idx=val1).astype(np.float16)
    #진동수렴 
    ...
    point_mean_values += np.random.normal(std,std,size=points.shape[0]) * mul
    anchors=[]
...
def increase_trend_partition(offset,points,val1,val2,logslope,variance):
    ...
    # point_mean_values = get_partition_points(val2-val1,count,offset_idx=val1).astype(np.float16)
...
def invert_date_candle(popen,pclose,next_day_open):
    """
        returns ndarray of length = DATE_LEN
    """
    variance = 0.7
    is_increasing = pclose > popen
    diff = abs(next_day_open - popen) * variance
    lower = popen if is_increasing else pclose
    split_point = int(interp_rand(0,DATE_LEN-1,0.5))
    
    if np.random.rand() < 0.3: is_increasing = not is_increasing
    # V shape
    if np.random.rand() > 0.5:
        anchor1 = np.random.triangular(lower-diff,lower,lower+diff)
        anchor3 = np.random.triangular(lower-diff*2,lower-diff,anchor1)
        bound = min(anchor1,anchor3)
        anchor2 = np.random.triangular(min(bound,lower-diff*3),min(bound,lower-diff*1.5),bound)
    else:
        #  ^ shape
        anchor1 = np.random.triangular(lower,lower+diff,lower+diff*2)
        anchor3 = np.random.triangular(lower-diff,lower,anchor1)
        bound = max(anchor1,anchor3)
        anchor2 = np.random.triangular(bound,max(bound,lower+diff),max(bound,lower+diff*2))

    if not is_increasing:
        anchor1,anchor3 = anchor3,anchor1
    
    result1 = np.linspace(anchor1,anchor2,split_point)
    result2 = np.linspace(anchor2,anchor3,DATE_LEN-split_point)

    assert result1.shape[0] + result2.shape[0] == DATE_LEN

    return np.concatenate((result1,result2))


def generate_v2(scale,variance,seed=None):
    if seed:
        np.random.seed(seed)
    ...
    trend_point_count = int(interp_rand(5,7,variance,std=0.5))
    ...
    for i in range(len(anchors3)-1):
        ...
        if length > 50:
            ...
            values += np.random.normal(0,0.02*(length/40),count)
    ...
    for i in range(len(anchors4)-1):
        ...
        #final noise
        result[pos1:pos1+length] = values

    for i in range(0,len(result)-DATE_LEN*2,DATE_LEN):
        popen = result[i]
        pclose = result[i+DATE_LEN-1]
        next_open=result[i+DATE_LEN]
        rand = np.random.rand() 
        if rand < 0.35: 
            result[i:i+DATE_LEN] = invert_date_candle(popen,pclose,next_open)
        elif rand < 0.8:
            diff = abs(pclose-popen)
            adjust_range=np.random.randint(3,10)
            adjust = np.linspace(diff/2,0,adjust_range)
            if np.random.rand() > 0.5:
                adjust *= -1
            result[i:i+adjust_range] += adjust
...
def test_stats():
    for var in [0,0.2,0.4,0.6,0.8,1]:
        ...
        delists=0
        for i in range(1000):
            arr,_,_,_=generate_v2(10,variance=var)
            maxes[i]=np.max(arr)
            if np.min(arr)<=0:
                delists +=1
        print(var)
